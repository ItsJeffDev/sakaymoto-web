...
export const api = {
  ...
  updateProfile: (body) => apiRequest('/users/me', { method: 'PATCH', body: JSON.stringify(body) }),
  motorcycles: (status = 'available') => apiRequest(`/motorcycles?status=${encodeURIComponent(status)}`),
  ...
  updateBookingStatus: (id, status) => apiRequest(`/bookings/${id}/status`, { method: 'PATCH', body: JSON.stringify({ status }) }),
  ...
  createMotorcycle: (body) => apiRequest('/motorcycles', { method: 'POST', body: JSON.stringify(body) }),
  updateMotorcycle: (id, body) => apiRequest(`/motorcycles/${id}`, { method: 'PATCH', body: JSON.stringify(body) }),
