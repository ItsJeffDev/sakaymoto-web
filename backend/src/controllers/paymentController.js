...
const getPayments = async (req, res) => {
  ...
  const [rows] = await db.execute(
    `SELECT p.*, b.user_id, b.motorcycle_id, b.start_date, b.end_date, u.name AS customer_name FROM payments p JOIN bookings b ON b.id = p.booking_id JOIN users u ON u.id = b.user_id ${isAdmin ? "" : "WHERE b.user_id = ?"} ORDER BY p.created_at DESC`,
    isAdmin ? [] : [req.user.id],
  );
...
const createPayment = async (req, res) => {
  ...
  if (Number(amount) > Number(bookings[0].total_price))
    return res
      .status(400)
      .json({ message: "Payment cannot exceed booking total" });
...
const updatePaymentStatus = async (req, res) => {
  ...
  const [result] = await db.execute(
    'UPDATE payments SET status = ?, paid_at = CASE WHEN ? = "verified" THEN CURRENT_TIMESTAMP ELSE paid_at END WHERE id = ?',
