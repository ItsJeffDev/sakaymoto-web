...
const uploadDocument = async (req, res) => {
  ...
  if (
    !req.file ||
    !["drivers_license", "valid_id", "other"].includes(document_type)
  )
    return res
      .status(400)
      .json({ message: "A valid document type and file are required" });
  const [result] = await db.execute(
    "INSERT INTO user_documents (user_id, document_type, file_url, expiration_date) VALUES (?, ?, ?, ?)",
    [
      req.user.id,
      document_type,
...
const updateDocumentStatus = async (req, res) => {
  ...
  const [result] = await db.execute(
    'UPDATE user_documents SET status = ?, verified_at = CASE WHEN ? = "verified" THEN CURRENT_TIMESTAMP ELSE NULL END WHERE id = ?',
